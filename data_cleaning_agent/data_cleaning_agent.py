...
def make_lightweight_data_cleaning_agent(
    ...
    def create_data_cleaner_code(state: GraphState):
        ...
        data_cleaning_prompt = PromptTemplate(
            ...
            - If the column is numeric, make sure you round the values to the nearest integer.Example: 18.34 should be rounded to 18 and 18.74 should be rounded to 19.
            - One important point to note is that when imputing values do not impute the values using the operation inPlace as it will modify the original dataframe.
            3. Remove duplicate rows and remove row with 50% missing values (This is important to remove the rows with high missing values as it will affect the analysis and model training).
            4. Use IQR to remove outliers

            Critical Instructions:
            - if the instructions from the user, conflict with the cleaning steps, then follow the instructions from the user which take priority. For example if the user has asked to not to remove outliers which is a part of the basic cleaning steps, then do not remove outliers.
            - If the user instructions specify filtering or any other taskthat does not affect the cleaning steps, then you MUST follow the instructions from the user. For example if the user has asked to filter records based on a condition provided by user_instructions, then perform the task after the basic cleaning steps have been applied. For example filtering by salary or age range.

            Coding Guidelines:
            - Use .copy() when you do slicing or filtering operations on a dataframe.
            - Use .loc[] when you assign values to rows or columns.
